...
# change this to "live" once you are sure everything is working correctly.
        ...
        # Get the parameters from the comandline input
        while getopts d:m: option
        do
         case "${option}"
         in
         d) REGISTER_DOMAINS=${OPTARG,,};;
         m) LETS_ENCRYPT_MODE=${OPTARG,,};;
         esac
        done

        # check if the 'd' option was provided
        if [ -z "$REGISTER_DOMAINS" ] ; then
                        echo "Must provide a domain with -d"; exit 1;
        fi

        # check if the 'm' option was provided and valid
        if [ -z "$LETS_ENCRYPT_MODE" ] ; then
                        echo "Must provide a mode of with -m"; exit 1;
        elif ! [[ ( "$LETS_ENCRYPT_MODE" == "staging" ) || ( "$LETS_ENCRYPT_MODE" == "live" ) ]]; then
                echo "Vaild Lets Encrypt modes are live or staging"; exit 1;
        fi
